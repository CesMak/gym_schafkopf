...
def learn_single(ppo, update_timestep, eps_decay, env, increase_batch, log_path, max_reward=-2.0):
    ...
    for uuu in range(0, 500000000+1):
        ...
        if uuu % eps_decay == 0:
            ppo.eps_clip *=0.8
            print("use eps decay: new one:"+str(round(ppo.eps_clip,3)))
...
if __name__ == '__main__':
    ...
    train_path     = os.path.abspath(os.getcwd()) + "/pretrained2.pth"
    nu_latent       = 128
    gamma           = 0.99
    K               = 16     #5
    increase_batch  = 3000    # value is multipled with 10=nu_remote!! increase batch size every update step! (instead of lowering lr), 100
    ...
    if train_from_start:
        ...
        eps = 0.1 # eps=0.2
        lr  = 0.01
        update_timestep = 5000 # 5000
        eps_decay       = update_timestep*10
        ppo = PPO(state_dim, action_dim, nu_cards, nu_latent, lr, (0.9, 0.999), gamma, K, eps)

    else:
        # setup learn further:
        eps = 0.01               #train further1: 0.05   train further2: 0.01 train further3:  0.001
        lr  = 0.002              #train further1: 0.005  train further2: 0.0009, train further3 0.0001
        update_timestep = 60000  # train further1: 30000  train further2: 180000  train further2: 300000
        increase_batch  = 5000
        eps_decay   = 10 # is currently not used!
        ppo = PPO(state_dim, action_dim, nu_cards, nu_latent, lr, (0.9, 0.999), gamma, K, eps)
